...
export function useMapNavigation(enabled: boolean, baseZoom: number, target: RefObject<Vector3>) {
  ...
  },[camera,cam,target,invalidate]);
  const zoom=useCallback((factor:number)=>{
    cam.zoom=baseZoom*clampZoom(cam.zoom/baseZoom*factor);
    cam.updateProjectionMatrix();move(new Vector3());report(cam.zoom/baseZoom,true);invalidate();
  },[cam,baseZoom,report,invalidate,move]);
  useEffect(()=>{report(enabled?cam.zoom/baseZoom:1,enabled);},[enabled,baseZoom,cam,report]);
  useEffect(()=>{
    if(!enabled)return;
    home.current.copy(camera.position).sub(target.current);move(new Vector3());
    ...
    const updates=frameTask(()=>{
      if(pending.size){
        ...
        const after=ground(midpoint());if(before&&after)move(before.sub(after));
      }
    ...
    const up=(e:PointerEvent)=>{updates.flush();pointers.delete(e.pointerId);if(el.hasPointerCapture(e.pointerId))el.releasePointerCapture(e.pointerId);if(!pointers.size)el.classList.remove("dragging");};
  ...
  },[enabled,camera,cam,baseZoom,gl,move,zoom,target]);
  useEffect(()=>{
    if(!enabled||!command)return;
    switch(command.type){
      case "in":zoom(1.25);break;
      case "out":zoom(.8);break;
      case "reset":target.current.set(...overview.target);camera.position.copy(home.current).add(target.current);cam.zoom=baseZoom;cam.updateProjectionMatrix();move(new Vector3());report(1,true);invalidate();break;
      default: {
