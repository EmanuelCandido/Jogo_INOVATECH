...
export function Shop() {
  ...
  return <section ref={panel} className="shop-screen" role="dialog" aria-modal="true" aria-labelledby="shop-heading" tabIndex={-1}>
    <div className="shop-layout" inert={confirmExit}>
      <header className="shop-topbar"><button className="shop-back" onClick={close} aria-label="Voltar ao mapa"><img src={publicAsset('/assets/ui/journey/shop-back.svg')} alt=""/></button><h2 id="shop-heading" className="sr-only">Loja do Impactus</h2><div className="balance shop-balance" aria-label={`${progress.coins} Moedas da Cidade`}><Coin/><b>{progress.coins.toLocaleString('pt-BR')}</b></div></header>
      ...
      <div className="shop-catalogue"><div className="shop-catalogue-scroll">
        <header className="shop-introduction"><h3>{categoryName}</h3><p>Escolha um acessório para o Impactus</p></header>
        <div className="accessory-tabs" role="tablist" aria-label="Categorias de acessórios" onKeyDown={event=>{
        ...
        <div id="accessory-collection" role="tabpanel" aria-labelledby={`tab-${category}`}>
          <div className="collection-heading"><b>{selected?.name??(category==='jacket'?'Sem jaqueta':'Sem chapéu')}</b><span>6 opções</span></div>
