...
export function surfaceShader(kind:SurfaceKind):Compiler{
 ...
 const compile:Compiler=shader=>{
  ...
  `).replace('#include <color_fragment>',`#include <color_fragment>
   ...
   float fine=(finishNoise(grainPoint)-.5)*(1.-smoothstep(.4,1.4,footprint));
   ...
   ${kind==='wood'?`float fibres=finishNoise(p*vec2(85.,2.)+vec2(finishNoise(p*3.)*2.,0.));
   surfaceTone+=(fibres-.5)*.06*(1.-smoothstep(.3,1.4,footprint));`:''}
   ...
   float grassFine=(finishNoise(blades+vec2(finishNoise(p*8.)*3.,0.))-.5)*(1.-smoothstep(.3,1.3,bladeAA));
   ...
   surfaceTone=(finishNoise(p*1.8)-.5)*.018+(finishNoise(blades)-.5)*.035*(1.-smoothstep(.25,1.2,bladeAA));`:''}
   ...
   float grit=(finishNoise(gritPoint)-.5)*(1.-smoothstep(.4,1.5,gritAA));
...
export function finishHardSurfaces(scene:Object3D){
 scene.traverse(o=>{
  ...
  for(const material of Array.isArray(o.material)?o.material:[o.material]){
   ...
   done.add(material);material.onBeforeCompile=surfaceShader(kind);material.customProgramCacheKey=()=>`surface-finish-v2-${kind}`;material.needsUpdate=true;
  }
 });
}
