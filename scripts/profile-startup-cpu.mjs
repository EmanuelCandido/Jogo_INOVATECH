...
const build=resolve('.tools/startup-profile'),out='docs/performance/startup-cpu-intel';await mkdir(out,{recursive:true});
const server=await preview({configFile:false,build:{outDir:build},preview:{host:'127.0.0.1',port:4187,strictPort:true}});
const browser=await chromium.launch({channel:'chrome',headless:true,args:['--enable-gpu','--use-angle=d3d11']});
const page=await browser.newPage({viewport:{width:1920,height:1080},deviceScaleFactor:1});page.setDefaultTimeout(180000);
...
try{
 ...
 await page.goto('http://127.0.0.1:4187/?benchmark=1',{waitUntil:'domcontentloaded'});
 await expect(page.getByRole('button',{name:'JOGAR',exact:true})).toBeEnabled({timeout:180000});
 await page.getByRole('button',{name:'JOGAR',exact:true}).click();await expect(page.getByRole('button',{name:'Continuar →',exact:true})).toBeVisible();
